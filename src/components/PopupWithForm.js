...
function PopupWithForm(props) {
   const {title, name, children, button, isOpen, onClose} = props;
    return (
        <div className={`popup popup_${name} ${isOpen ? 'popup_opened' : ''}`}>
            <div className="popup__container">
                ...
                <form className={`popup__form popup__form_${name}`}>
                    ...
                    {children}
                    <button
                        className="popup__button popup__button_disabled popup__button_active_submit popup__button_active_save-profile"
